...
function isOriginAllowed(origin) {
    ...
    if (CONFIG.ALLOWED_ORIGINS.length && !CONFIG.ALLOWED_ORIGINS.includes(origin)) return false;
    return true;
}

function buildUpstreamHeaders(req, url, headersParam, passedCookies, isPlaylist) {
    const headers = {
    ...
    // FIX: Only forward Range headers for actual video chunks/keys.
    // If a native player asks for a byte-range of the playlist text file, strip it to prevent freezing.
    if (req.headers.range && !isPlaylist) {
    ...
    if (passedCookies) {
        headers['cookie'] = headers['cookie'] ? `${headers['cookie']}; ${passedCookies}` : passedCookies;
    }
...
app.get('/', (req, res) => {
    ...
    if (!isOriginAllowed(origin)) {
        res.status(403).send(`Origin blacklisted.`);
        return;
    }
...
app.get("/m3u8-proxy", async (req, res) => {
    ...
    if (!isOriginAllowed(origin)) return safeSend(403, `Origin blacklisted.`);
    ...
    try {
        ...
        const headers = buildUpstreamHeaders(req, url, headersParam, passedCookies, isPlaylist);

        setCorsHeaders(req, res);

        if (isPlaylist) {
            // Force Vercel to completely drop the incoming range check for the playlist configuration
            ...
            try {
                ...
                // Force a clean 200 OK state so native players read the whole file instead of freezing
            ...
            const proxyRequest = https.get(url.href, { headers, rejectUnauthorized: false }, (targetResponse) => {
                if (targetResponse.statusCode >= 400) {
                    return safeSend(targetResponse.statusCode, { message: `Upstream error: ${targetResponse.statusCode}` });
                }

                const streamHeaders = ['content-type', 'content-length', 'content-range', 'accept-ranges'];
                Object.entries(targetResponse.headers).forEach(([k, v]) => {
                    const lk = k.toLowerCase();
                    if (streamHeaders.includes(lk)) res.setHeader(k, v);
                });
        ...
        if (!res.headersSent) safeSend(500, { message: e.message });
    }
});
